export const delegation = 'https://learn.javascript.ru/event-delegation'


// Что такое делегирование событий в JS
// простой метод, с помощью которого ты добавляешь один обработчик событий к родительскому элементу, чтобы избежать 
// необходимости добавлять обработчики событий к нескольким дочерним элементам.
// используя делегирование событий, можно добавить обработчик событий к элементу, дождаться, 
// пока событие всплывет из дочернего элемента, и тогда легко определить, из какого элемента возникло событие.
...
// Во-первых, событие должно всплывать. Некоторые события этого не делают. Также, низкоуровневые обработчики 
// не должны вызывать event.stopPropagation().
...
// а не document.on<событие>, т.к. в случае последнего могут возникать конфликты: новые обработчики будут 
// перезаписывать уже существующие.
