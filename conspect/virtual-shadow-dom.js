...
// Virtual DOM (VDOM) - это представление Real DOM, хранимое в оперативной памяти. Это представление 
//синхронизируется с "настоящим" DOM. Сравнение происходит между вызовом функции рендеринга и отображением 
//элемента на экране. Данный внутренний процесс называется reconciliation (согласованием).
...
// Виртуальный DOM - это асбтракция над браузерным DOM. Виртуальный DOM позволяет избежать внесения 
//ненужных изменений в DOM, которые являются "дорогими", поскольку изменения DOM, обычно, приводят 
//к повторному рендерингу страницы. Он позволяет объединять несколько обновлений в одно, 
//поэтому не каждое изменение приводит к повторному рендерингу, вместо этого, перерисовка браузерного 
//DOM осуществляется только после применения всех изменений к виртуальному DOM и определения различий между ними.
// Изменения к браузерному DOM применяются частично.
...
// shadowRoot = elem.attachShadow({mode: open|closed}) – создаёт теневой DOM для elem. 
//Если mode="open", он доступен через свойство elem.shadowRoot.
