export const useStrict = 'https://learn.javascript.ru/strict-mode'
...
//застревали в языке навсегда.

// Так было до 2009 года, когда появился ECMAScript 5 (ES5). Он добавил новые возможности в язык 
//и изменил некоторые из существующих. Чтобы устаревший код работал, как и раньше, по умолчанию 
//подобные изменения не применяются. Поэтому нам нужно явно их активировать 
...
//всего скрипта "use strict" можно поставить в начале большинства видов функций. 
//Это позволяет включить строгий режим только в конкретной функции. Но обычно 
//люди используют его для всего файла.
...
// Консоль браузера
// В дальнейшем, когда вы будете использовать консоль браузера для тестирования функций, 
//обратите внимание, что use strict по умолчанию в ней выключен.
...
//(и мы, конечно, до них доберёмся), которые автоматически включают строгий режим. 
...
// В следующих главах, по мере расширения знаний о возможностях языка, мы яснее 
//увидим отличия между строгим и стандартным режимом. К счастью, их не так много, 
//и все они делают жизнь разработчика лучше.
...
//Будьте внимательны: при объединении со скриптами, использующими строгий режим, скрипты, 
//которые его не используют, начинают его использовать, и наоборот, поэтому объединять 
//скрипты с разными режимами не стоит.
